...
void WriteRecordsToFile()
{
    ...
    while(true)
    {
        Console.Write("Enter the name of the file to save (must end with .csv): ");
...
void ReadRecordsToList()
{
    ...
    while(true)
    {
        Console.Write("Enter the name of the file to load (must end with .csv): ");
        filename = Console.ReadLine();       
    ...
    {
        //  Clear the game sales list first before you load so that it doesn't append
        gameSales = new();

        //  Skip the first line because it is headers
        reader.ReadLine();
        ...
        while(!reader.EndOfStream)
        {
            ...
            gameSales.Add(
                new GameSale(line[0], line[1], line[2], int.Parse(line[3]), double.Parse(line[4]))
            );
        }
